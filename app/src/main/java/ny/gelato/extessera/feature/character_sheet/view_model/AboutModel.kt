...
data class AboutModel(
    ...
    constructor(character: Character) :
            this(character.name,
                    ...
                    character.traits.map { it.name }
                            .toString()
                            .substring(1)
                            .dropLast(1),

                    character.primary.features.map { it.name }
                            .toString()
                            .substring(1)
                            .dropLast(1),

                    if (character.proficiencies.map { it.name }.containsAll(Weapon.Type.values()
                            .map { it.formatted }))
                        "All Weapons"
                    else if (character.proficiencies.map { it.name }.containsAll(Weapon.Type.values()
                            .filter { it.isSimple() }
                            .map { it.formatted }))
                        character.proficiencies.filter {
                            it.type == Proficiency.Type.WEAPON.name
                                ...
                                .substring(1)
                                .dropLast(1)
                    else character.proficiencies.filter { it.type == Proficiency.Type.WEAPON.name }
                            .map { if (it.name.contains(",")) it.name.replace(", ", " (").plus(")") else it.name }
                            .toString()
                            .substring(1)
                            .dropLast(1),

                    character.proficiencies.filter { it.type == Proficiency.Type.ARMOR.name }
                            .map { it.name }
                            .toString()
                            .substring(1)
                            .dropLast(1),

                    character.proficiencies.filter { it.type == Proficiency.Type.TOOL.name }
                            .map { if (it.name.contains(",")) it.name.replace(", ", " (").plus(")") else it.name }
                            .toString()
                            .substring(1)
                            .dropLast(1),

                    character.proficiencies.filter { it.type == Proficiency.Type.LANGUAGE.name }
                            .map { if (it.name.contains(",")) it.name.replace(", ", " (").plus(")") else it.name }
                            .toString()
                            .substring(1)
                            .dropLast(1))
